...
public class RandomImpl implements Random {
    ...
    public RandomImpl() {
        ...
        this.picEvolve = new PicEvolve(this);
    }

    @Override
    public ScalarNode nextScalar() {
        return new ScalarNode(this.random.nextDouble());
    }

    @Override
    public VectorNode nextVector() {
        return new VectorNode(
                new ArrayList<>(Arrays.asList(this.random.nextDouble(),
    ...
    public Expression nextTerminal() {
        ...
        if (rand >= 0 && rand < (1 / 3.0)) {
            ...
            return new VariableNode(
                    this.random.nextDouble() < 0.5 ? Variable.X : Variable.Y);
        }
    }

    @Override
    public Function nextFunction() {
        final Iterator<String> iter = this.picEvolve.getFunctionNames()
                .iterator();
        IntStream
                .range(0,
                        this.random.nextInt(
                                this.picEvolve.getFunctionNames().size()))
                .forEach(i -> iter.next());

        return this.picEvolve.getFunction(iter.next()).get();
    }

    /**
     * Generates a random s-expression. Chooses a random function from the
     * function set and then generates the required number of arguments, each of
    ...
    public Expression nextExpression() {
        ...
        final Function randFunc = this.nextFunction();
        ...
        for (int i = 0; i < randFunc.getArity(); i++) {
            ...
            if (rand >= 0 && rand < 0.25) {
                sb.append(this.random.nextDouble());
            } else if (rand >= 0.25 && rand < 0.50) {
                sb.append("#").append(this.random.nextDouble()).append(",")
                        .append(this.random.nextDouble()).append(",")
                        .append(this.random.nextDouble());
            } else if (rand >= 0.50 && rand < 0.75) {
                sb.append(this.random.nextDouble() > 0.5 ? Variable.X
                        : Variable.Y);
            } else {
                sb.append(this.nextExpression());
            }
            sb.append(" ");
        }
        final String exprStr = sb.toString().trim() + ")";

        return this.picEvolve.parse(exprStr);
    }

    /**
     * Uses reservoir sampling algorithm to choose random node from
    ...
    public Expression nextNode(final Expression root) {
        ...
        while (!exprStack.isEmpty()) {
            ...
            if (this.random.nextDouble() < (1.0 / (i++))) {
                randomNode = curNode;
            }
    ...
    public Expression nextChild(final Expression node) {
        return node.getChildren()
                .get(this.random.nextInt(node.getChildren().size()));
    }

    @Override
    public Mutation nextMutation(final Class<? extends Expression> nodeType) {

        final List<MutationFrequency> freqs = new MutationFactory(this)
                .getMutationFrequencies(nodeType);

        final int totalWeight = freqs.stream()
                .mapToInt(freq -> freq.getRelativeFrequency()).sum();

        final int rand = this.random.nextInt(totalWeight);
    ...
    /**
     * Scales overall mutation frequency inversely in proportion to length of
     * parent expression
     */
    @Override
    public boolean shouldMutate(final Expression expression,
            final double globalMutationFrequency) {
        return this.random.nextDouble() < (globalMutationFrequency
                * (1.0 / this.getHeight(expression)));
    }
