...
class GimpGradient:
        ...
        def colorRGB(self, scalar):
                """Wrapper for color method that returns
                   r,g,b integers scaled 0-255."""

                color = list(self.color(scalar))
        ...
        class GgrView(wx.Frame):
                ...
                def paint_some(self, dc, chunks, y0, y1):
                        ...
                        for x in range(0, cw, chunkw):
                                c = map(lambda x:int(255*x), self.ggr.color(float(x)/cw))
